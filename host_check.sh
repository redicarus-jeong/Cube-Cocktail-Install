...
echo ">>> ${MASTER_COUNT} / ${MASTER_HOSTNAME[@]}"

if [ ${MASTER_COUNT} -eq 0 ]; then
   echo ">>> Not Found Master Hostname in  cube.env file"
   echo; exit 0
fi
...
CNT=0
for HOST_NAME in ${MASTER_HOSTNAME[*]}
  do
    if [ "${HOSTNAME}" = "${HOST_NAME}" ]; then
      ...
      echo "This Server Name is \"${HOSTNAME}\" Check this server hostname"
      echo "Set hostname is \"hostnamectl set-hostname <your host name>\""
      exit 0
    fi
    (( CNT += 1 ))
done
